...
router.get('/:id', (req,res) => {
    ...
    responseHTML += `<div class="container" style="text-align:center"><p><pre><code><u>Logs</u><br><br>`

    URL.find({shortURL: req.params.id})
    .then((urlFound)=>{
        if(urlFound.length > 0)
        {
            responseHTML += ` Long URL is ${urlFound[0].longURL} <br> Short URL is https://gburlshort.herokuapp.com/${urlFound[0].shortURL} <br> It was accessed ${urlFound[0].accessCounts} times <br> Created on ${new Date(urlFound[0].timeOfCreation * 1000)} <br> and will expire on ${new Date(urlFound[0].timeOfDeletion * 1000)}`;
